...
#' @description Main retrieval function for RNA sequences of an organism of interest.
#' By specifying the scientific name of an organism of interest the corresponding fasta-file storing the RNA information for the organism of interest
#' can be downloaded and stored locally. RNA files can be retrieved from several databases.
...
#' CDS file shall be stored. Default is \code{path} = \code{file.path("_ncbi_downloads","RNA")}.
...
#' 
#' }
#' @seealso \code{\link{getGenome}}, \code{\link{getProteome}}, \code{\link{getRNA}}, \code{\link{getGFF}}, \code{\link{meta.retrieval}}, \code{\link{read_cds}}
#' @export

getRNA <- function(db = "refseq", organism, path = file.path("_ncbi_downloads","RNA")){
    ...
    if (is.element(db, c("refseq", "genbank"))) {
        ...
        if (nrow(FoundOrganism) == 0) {
            ...
            if (!exists.ftp.file(url = paste0(FoundOrganism$ftp_path,"/"), file.path = download_url)) {
                message("Unfortunately no RNA file could be found for organism '",organism,"'. Thus, the download of this organism has been omitted.")
                return(FALSE) 
            }
    ...
    if (db == "ensembl") {
        ...
        rna.path <- getENSEMBL.Seq(organism, type = "ncrna", id.type = "none", path)
    ...
    if (db == "ensemblgenomes") {
        ...
        rna.path <- getENSEMBLGENOMES.Seq(organism, type = "ncrna", id.type = "none", path)
