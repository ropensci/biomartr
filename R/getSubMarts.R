...
getSubMarts <- function(submart = "ensembl") {
    ...
    if (!is.element(submart, c("ensembl", "plants", "fungi", "protists", "metazoa")))
        stop("Please select a submart that is supported by ensembl: submart = 'ensembl', submart = 'plants', submart = 'fungi',  submart = 'protists', or submart = 'metazoa'", call. = FALSE)
    ...
    if (submart == "plants")
        # connect to BioMart API
        biomartPage <- httr::handle("http://plants.ensembl.org:80/biomart/martservice?type=registry&requestid=biomart")
    
    if (submart == "fungi")
        # connect to BioMart API
        biomartPage <- httr::handle("http://fungi.ensembl.org:80/biomart/martservice?type=registry&requestid=biomart")
    
    if (submart == "protists")
        # connect to BioMart API
        biomartPage <- httr::handle("http://protists.ensembl.org:80/biomart/martservice?type=registry&requestid=biomart")
