...
#' @description Retrieve available annotation *.gff files for metagenomes from NCBI Genbank. NCBI Genbank allows users
...
getMetaGenomeAnnotations <- function(name, path = file.path("_ncbi_downloads","metagenome","annotations")) {
    ...
    for (i in seq_len(length(file.names))) {
        ...
        tryCatch({utils::capture.output(downloader::download(
            download_url,
            destfile = file.path(path, paste0(basename(file.names[i]), "_genomic.gff.gz")),
            mode = "wb"
        ))}, error = function(e)
            stop(
