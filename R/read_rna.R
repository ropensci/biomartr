...
#' @description This function reads an organism specific RNA stored in a defined file format.
#' @param file a character string specifying the path to the file storing the RNA.
...
#' @param delete_corrupt a logical value specifying whether potential RNA sequences that cannot be divided by 3 shall be
...
#' of interest as first argument. It is possible to read in different proteome file standards such as \emph{fasta} or \emph{genebank}.
#'
#' @return A data.table storing the gene id in the first column and the corresponding
#' sequence as string in the second column.
#' @seealso \code{\link{getRNA}}, \code{\link{read_genome}}, \code{\link{read_proteome}}, \code{\link{read_gff}}
#' @import Biostrings
#' @export
read_rna <- function(file, format = "fasta", obj.type = "Biostrings", delete_corrupt = FALSE, ...){
        ...
        tryCatch({
            ...
            RNA.dt <-
                data.table::data.table(geneids = RNA_names ,
                                       seqs = tolower(as.character(RNA_file)))
