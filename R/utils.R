...
is.genome.available <- function(organism, details = FALSE, database = "refseq"){
    ...
    if (!is.element(database, c("refseq", "genbank")))
        stop("Please select one of the available data bases: 'refseq' or 'genbank'", call. = FALSE)
    ...
    if (nrow(FoundOrganism) == 0)
        stop("Unfortunately no entry for organism '",organism,"' could not be found.", call. = FALSE)
