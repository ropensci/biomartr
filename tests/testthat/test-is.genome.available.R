...
test_that("The is.genome.available() error messages work properly..",{
    ...
    expect_error(is.genome.available(organism = "Hrmo sapi", db = "refseq"), "Unfortunately no entry for organism 'Hrmo sapi' could be found.")
    # test unknown organism error handling for 'genbank'
    expect_error(is.genome.available(organism = "Hrmo sapi", db = "genbank"), "Unfortunately no entry for organism 'Hrmo sapi' could be found.")
