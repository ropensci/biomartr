...
Hence, currently 15512 genomes (including all kingdoms of life) are stored on [NCBI](ftp://ftp.ncbi.nlm.nih.gov/genomes/GENOME_REPORTS/overview.txt) servers.
...
```{r,eval=FALSE}
# download all vertebrate genomes
meta.retrieval(kingdom = "vertebrate_mammalian", type = "proteome")
```

#### Retrieve CDS Sequences

Download all mammalian vertebrate CDS from RefSeq.
...
Alternative kingdoms can be obtained with `getKingdoms()`.
