...
The following functions allow you to download proteomes, genomes, and CDS files from several
database resources such as: `refseq`. When a correspondning proteome, genome, or CDS file was
...
The `getGenome()` function creates a directory named `_ncbi_downloads/genomes` in which corresponding genome named `Arabidopsis_thaliana_genome.fna.gz` is downloaded. The `read_genome()` function enables you to work with the genome as `data.table` object.
...
In case you would like to store the genome file at a different location,
you can specify the `path = file.path("put","your","path","here")` argument.
...
The `getProteome()` function creates a directory named `_ncbi_downloads/proteomes` in which corresponding proteome named `Arabidopsis_thaliana_protein.faa.gz` is downloaded. The `read_proteome()` function enables you to work with the proteome as `data.table` object.
...
In case you would like to store the proteome file at a different location,
you can specify the `path = file.path("put","your","path","here")` argument.
...
The `getCDS()` function creates a directory named `_ncbi_downloads/CDS` in which
...
In case you would like to store the CDS file at a different location,
you can specify the `path = file.path("put","your","path","here")` argument.
...
1            tair_locus            TAIR locus ID(s) plants_mart_25 athaliana_eg_gene
2      tair_locus_model      TAIR locus model ID(s) plants_mart_25 athaliana_eg_gene
3           tair_symbol           TAIR symbol ID(s) plants_mart_25 athaliana_eg_gene
4       with_tair_locus       with TAIR locus ID(s) plants_mart_25 athaliana_eg_gene
5 with_tair_locus_model with TAIR locus model ID(s) plants_mart_25 athaliana_eg_gene
6      with_tair_symbol      with TAIR symbol ID(s) plants_mart_25 athaliana_eg_gene
```

As you can see, we need the filter argument `tair_locus` from `mart` `plants_mart_25` and `dataset` `athaliana_eg_gene`.
...
Source: local data frame [3 x 4]
            ...
            name       description           mart           dataset
1    refseq_mrna    RefSeq mRNA ID plants_mart_25 athaliana_eg_gene
2   refseq_ncrna   RefSeq ncRNA ID plants_mart_25 athaliana_eg_gene
3 refseq_peptide RefSeq protein ID plants_mart_25 athaliana_eg_gene
```

Now it is clear that we need attribute argument `refseq_peptide` to get the
refseq id of the corresponding filter `tair_locus` ids.
