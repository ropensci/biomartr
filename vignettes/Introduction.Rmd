...
Users of the [myTAI](https://github.com/HajkD/myTAI) and [orthologr](https://github.com/HajkD/orthologr) packages will find additional functions allowing them to integrate `biomartr` to any workflow implemented using `myTAI` or `orthologr`. The `biomartr` package is designed to achieve the highest degree of reproducible _evolutionary transcriptomics_ research from data retrieval to data visualization.
