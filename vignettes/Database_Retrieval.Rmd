...
First, the `listDatabases(db_name = "nr")` allows to list all files corresponding to the `nr` database. 
...
In a next step users can use a combination of the `listDatabases()` and `download_databases()` functions to retrieve 
all files corresponding to a specific NCBI database.

## Download available databases

Using the same search strategy by specifying the database name as described above, users can now download these databases using the `download_database()` function.
