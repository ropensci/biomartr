...
5          fungi_mart_24             ENSEMBL FUNGI 23 (EBI UK)
6    fungi_variations_24   ENSEMBL FUNGI VARIATION 23 (EBI UK)
7        metazoa_mart_24           ENSEMBL METAZOA 23 (EBI UK)
8  metazoa_variations_24 ENSEMBL METAZOA VARIATION 23 (EBI UK)
9         plants_mart_24            ENSEMBL PLANTS 23 (EBI UK)
10  plants_variations_24  ENSEMBL PLANTS VARIATION 23 (EBI UK)
```
...
For our example, we will choose the `plants_variations_24` `mart` and list all available
datasets that are element of this `mart`.

```{r,eval=FALSE}

tail(listDatasets(useMart("plants_mart_24")), 10)
...
head(listAttributes(useDataset(dataset = "athaliana_eg_gene", mart = useMart("plants_mart_24"))), 10)
```
...
head(listFilters(useDataset(dataset = "athaliana_eg_gene", mart = useMart("plants_mart_24"))), 10)
```
...
mart <- useDataset("athaliana_eg_gene", mart = useMart("plants_mart_24"))
...
query methodology that aims to be more organism centric. Another aspect of useability is `package dependencies`.
The `biomaRt` package has been stored on [Bioconductor](http://biomart.org/) and therefore, additional work has to be done to enable dependencies in [CRAN](http://cran.r-project.org/) packages that rely on [Bioconductor](http://biomart.org/) packages.
Therefore, `biomartr` provides new BioMart interface functions that were implemented based on base R packages or packages
stored on [CRAN](http://cran.r-project.org/).

## Retrieve marts, datasets, attributes, and filters with biomartr

### Retrieve Available Marts

The `getMarts()` function allows you to list all available databases that cann be accessed through BioMart interfaces.
...
5          fungi_mart_24             ENSEMBL FUNGI 24 (EBI UK)
6    fungi_variations_24   ENSEMBL FUNGI VARIATION 24 (EBI UK)
7        metazoa_mart_24           ENSEMBL METAZOA 24 (EBI UK)
8  metazoa_variations_24 ENSEMBL METAZOA VARIATION 24 (EBI UK)
9         plants_mart_24            ENSEMBL PLANTS 24 (EBI UK)
10  plants_variations_24  ENSEMBL PLANTS VARIATION 24 (EBI UK)
```

### Retrieve Available Datasets from a Specific Mart

Now you can select a specific database to list all available datasets
that can be accessed trhough this database. In this example we choose
the `plants_mart_24` database.

```{r,eval=FALSE}
head(getDatasets(mart = "plants_mart_24") , 5)
```
...
```{r,eval=FALSE}
tail(getDatasets(mart = "plants_mart_24") , 3)
```
...
Now that you have selected a database (`plants_mart_24`) and a dataset (`athaliana_eg_gene`),
...
head( getAttributes(mart = "plants_mart_24", dataset = "athaliana_eg_gene"), 10 )
```
...
head( getFilters(mart = "plants_mart_24", dataset = "athaliana_eg_gene"), 10 )
```
...
1                             3_utr_end                                  3' UTR end plants_mart_23 athaliana_eg_gene
2                           3_utr_start                                3' UTR start plants_mart_23 athaliana_eg_gene
3                                  3utr                                      3' UTR plants_mart_23 athaliana_eg_gene
4                             5_utr_end                                  5' UTR end plants_mart_23 athaliana_eg_gene
5                           5_utr_start                                5' UTR start plants_mart_23 athaliana_eg_gene
6                                  5utr                                      5' UTR plants_mart_23 athaliana_eg_gene
7                      affy_ath1_121501 Affymetrix array Arabidopsis ATH1 121501 ID plants_mart_23 athaliana_eg_gene
8                                allele                              Variant allele plants_mart_23  athaliana_eg_snp
9                  alyrata_eg_chrom_end                 Arabidopsis lyrata end (bp) plants_mart_23 athaliana_eg_gene
10               alyrata_eg_chrom_start               Arabidopsis lyrata start (bp) plants_mart_23 athaliana_eg_gene
11                alyrata_eg_chromosome      Arabidopsis lyrata chromosome/scaffold plants_mart_23 athaliana_eg_gene
12                      alyrata_eg_gene           Arabidopsis lyrata gene stable ID plants_mart_23 athaliana_eg_gene
13          alyrata_eg_homolog_ancestor                                    Ancestor plants_mart_23 athaliana_eg_gene
14                alyrata_eg_homolog_dn                                          dS plants_mart_23 athaliana_eg_gene
15                alyrata_eg_homolog_ds                                          dN plants_mart_23 athaliana_eg_gene
16   alyrata_eg_homolog_ensembl_peptide        Arabidopsis lyrata protein stable ID plants_mart_23 athaliana_eg_gene
17 alyrata_eg_homolog_is_tree_compliant        Orthology confidence [0 low, 1 high] plants_mart_23 athaliana_eg_gene
18           alyrata_eg_homolog_perc_id                                  % identity plants_mart_23 athaliana_eg_gene
19        alyrata_eg_homolog_perc_id_r1               Arabidopsis lyrata % identity plants_mart_23 athaliana_eg_gene
20            alyrata_eg_orthology_type                               Homology type plants_mart_23 athaliana_eg_gene
```
...
11            atrichopoda_eg_homolog_perc_id_r1           Amborella trichopoda % identity plants_mart_23 athaliana_eg_gene
12       bdistachyon_eg_homolog_ensembl_peptide Brachypodium distachyon protein stable ID plants_mart_23 athaliana_eg_gene
13               bdistachyon_eg_homolog_perc_id                                % identity plants_mart_23 athaliana_eg_gene
14            bdistachyon_eg_homolog_perc_id_r1        Brachypodium distachyon % identity plants_mart_23 athaliana_eg_gene
15         boleracea_eg_homolog_ensembl_peptide       Brassica oleracea protein stable ID plants_mart_23 athaliana_eg_gene
16                 boleracea_eg_homolog_perc_id                                % identity plants_mart_23 athaliana_eg_gene
17              boleracea_eg_homolog_perc_id_r1              Brassica oleracea % identity plants_mart_23 athaliana_eg_gene
18             brapa_eg_homolog_ensembl_peptide           Brassica rapa protein stable ID plants_mart_23 athaliana_eg_gene
19                     brapa_eg_homolog_perc_id                                % identity plants_mart_23 athaliana_eg_gene
20                  brapa_eg_homolog_perc_id_r1                  Brassica rapa % identity plants_mart_23 athaliana_eg_gene
```
...
Taken together, the following workflow allows you to perform fast BioMart queries for 
...
__Note:__ dataset names change very frequently due to the update of dataset versions.
So in case some query functions do not work properly, you should check with
`organismAttributes(update = TRUE)` whether or not your dataset name has been changed.
For example, `organismAttributes("Arabidopsis thaliana", topic = "id", update = TRUE)`
might reveal that the dataset `plants_mart_23` has changed to `plants_mart_24`.


## Gene Ontology 

The `biomartr` package also enables a fast and intuitive retrieval of GO terms
and additional information via the `getGO()` function.
...
```
  tair_locus go_accession goslim_goa_accession goslim_goa_description
1  AT1G06090   GO:0016717           GO:0005783  endoplasmic reticulum
2  AT1G06090   GO:0006631           GO:0005783  endoplasmic reticulum
3  AT1G06090   GO:0006636           GO:0005783  endoplasmic reticulum
4  AT1G06090   GO:0005789           GO:0005783  endoplasmic reticulum
5  AT1G06090   GO:0005783           GO:0005783  endoplasmic reticulum
6  AT1G06090   GO:0005575           GO:0005783  endoplasmic reticulum
```
