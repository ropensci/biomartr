...
package and will then introduce you to the interface functions `biomart()` and `biomart_organisms()` implemented in `biomartr`
...
For our example, we will choose the `plants_variations_25` `mart` and list all available
datasets that are element of this `mart`.

```{r,eval=FALSE}

tail(listDatasets(useMart("plants_mart_25")), 10)
...
head(listAttributes(useDataset(dataset = "athaliana_eg_gene", mart = useMart("plants_mart_25"))), 10)
```
