...
#### Example retrieval of all `Gammaproteobacteria` genomes from `NCBI ReSeq`:

Please note that the following functionality is so far only provided by the [developer version of biomartr](https://github.com/HajkD/biomartr#download-developer-version).
...
#### Example retrieval of all `Adenoviridae` genomes from `NCBI ReSeq`:
...
Again, by exchanging `type = "genome"` by either `type = "proteome"`, `type = "CDS"`, or `type = "gff"`, users can retrieve proteome, CDS, or GFF files instead of genomes.
